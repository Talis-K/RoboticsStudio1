...
# Checks if the path intersects an object cluster before planning a detour.
#give start x and y, goal x and y, object centre x and y, and radius
def path_intersects_circle(start, goal, centre, radius, n_points=30):
    sx, sy = start; gx, gy = goal; cx, cy = centre
    xs = np.linspace(sx, gx, n_points)
    ys = np.linspace(sy, gy, n_points)
    d = np.hypot(xs - cx, ys - cy)

    #if intersection is true, then produce waypoints and publish them to avoidance_waypoints
    # not sure which function handles this
...
# Quaternion → yaw conversion helper. Odometry stores orientation as a quaternion; we only need planar yaw for planning/logging.
def q_to_yaw(q):
...
# Checks that angles are correct
def angle_diff(a, b):
    return angle_wrap(a - b)



# Compute tangent points from a current to target waypoint
def tangent_points(px, py, cx, cy, R):
    dx, dy = px - cx, py - cy
    d = math.hypot(dx, dy)
    if d <= R:  # no tangents if inside/at circle
        return False, None, None, None, None
    theta = math.atan2(dy, dx)
    # Offset angle from centre→point line to tangent contact.
    delta = math.acos(R / d)
    phi_p = theta + delta   # CCW tangent
    phi_m = theta - delta   # CW  tangent
    T_p = (cx + R * math.cos(phi_p), cy + R * math.sin(phi_p))
    T_m = (cx + R * math.cos(phi_m), cy + R * math.sin(phi_m))
    return True, T_p, T_m, phi_p, phi_m

# create points along the arc
def sample_arc(cx, cy, R, phi1, phi2, ccw=True, n=20):
    pts = []
    dphi = angle_diff(phi2, phi1)
    if ccw and dphi < 0: dphi += 2 * math.pi
    if not ccw and dphi > 0: dphi -= 2 * math.pi
    # Divide arc into n+1 segments and take interior points.
    step = dphi / (n + 1)
...
# Generate path on counter clockwise side
def generate_avoidance_waypoints(curr_xy, next_xy, centre, radius, z=0.5):
    sx, sy = curr_xy; gx, gy = next_xy; cx, cy = centre; R = radius
    okS, Ts_p, Ts_m, phis_p, phis_m = tangent_points(sx, sy, cx, cy, R)
    okG, Tg_p, Tg_m, phig_p, phig_m = tangent_points(gx, gy, cx, cy, R)
    # if inside circle, follow straight line
    if not okS or not okG:
        return [(sx, sy, z), (gx, gy, z)]
    Ts, Tg, phis, phig, ccw = Ts_p, Tg_p, phis_p, phig_p, True
    # Compose path: start → start-tangent → arc points → goal-tangent → goal.
    path2d = [(sx, sy), Ts]
    path2d += sample_arc(cx, cy, R, phis, phig, ccw, n=20)
    path2d += [Tg, (gx, gy)]
    return [(x, y, z) for x, y in path2d]
...
class ObstacleBypassPlanner(Node):
    def __init__(self):
        ...
        # I/O
        self.sub_obj = self.create_subscription(Float32MultiArray, '/object_geometry', self.on_object, 10)
    ...
    def on_object(self, msg: Float32MultiArray):




        #This shpould itteratrate through each geometry and check for intersections
